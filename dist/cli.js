  ...
  "node_modules/picomatch/lib/picomatch.js"(exports, module) {
    ...
    var picomatch3 = (glob2, options, returnState = false) => {
      ...
      const matcher = (input, returnObject = false) => {
        const { isMatch, match, output } = picomatch3.test(input, regex2, options, { glob: glob2, posix: posix2 });
    ...
    picomatch3.test = (input, regex2, options, { glob: glob2, posix: posix2 } = {}) => {
      if (typeof input !== "string") {
      ...
      if (match === false || opts.capture === true) {
        if (opts.matchBase === true || opts.basename === true) {
          match = picomatch3.matchBase(input, regex2, options, posix2);
        } else {
...
function joinPathWithRelativePath(root2, options) {
...
function build$7(root2, options) {
  const { relativePaths, includeBasePath } = options;
  return relativePaths && root2 ? joinPathWithRelativePath(root2, options) : includeBasePath ? joinPathWithBasePath : joinPath;
}
function pushDirectoryWithRelativePath(root2) {
  return function(directoryPath, paths) {
    paths.push(directoryPath.substring(root2.length) || ".");
  };
}
function pushDirectoryFilterWithRelativePath(root2) {
  return function(directoryPath, paths, filters) {
    const relativePath = directoryPath.substring(root2.length) || ".";
...
function build$6(root2, options) {
  const { includeDirs, filters, relativePaths } = options;
  if (!includeDirs) return empty$2;
  if (relativePaths) return filters && filters.length ? pushDirectoryFilterWithRelativePath(root2) : pushDirectoryWithRelativePath(root2);
...
var Walker = class {
  ...
  walkDirectory;
  callbackInvoker;
  constructor(root2, options, callback$1) {
    this.isSynchronous = !callback$1;
    this.callbackInvoker = build$1(options, this.isSynchronous);
    this.root = normalizePath(root2, options);
    this.state = {
...
function promise(root2, options) {
  return new Promise((resolve$1, reject) => {
    callback(root2, options, (err, output) => {
      if (err) return reject(err);
      resolve$1(output);
    });
  });
}
function callback(root2, options, callback$1) {
  let walker = new Walker(root2, options, callback$1);
  walker.start();
}
function sync(root2, options) {
...
var Builder = class {
  ...
  crawl(root2) {
    return new APIBuilder(root2 || ".", this.options);
  }
  withGlobFunction(fn) {
  ...
  */
  /* c8 ignore next 4 */
  crawlWithOptions(root2, options) {
    this.options = {
      ...this.options,
      ...options
    };
...
function buildFormat(cwd, root2, absolute) {
  ...
  return (p) => posix.relative(cwd, `${root2}/${p}`) || ".";
}
function buildRelative(cwd, root2) {
  if (root2.startsWith(`${cwd}/`)) {
    const prefix = root2.slice(cwd.length + 1);
    return (p) => `${prefix}/${p}`;
  }
  return (p) => {
    const result = posix.relative(cwd, `${root2}/${p}`);
    return p[p.length - 1] === "/" && result !== "" ? `${result}/` : result || ".";
  };
}
function ensureNonDriveRelativePath(path2) {
  return path2.replace(DRIVE_RELATIVE_PATH, (match) => `${match}/`);
}
...
async function glob(globInput, options) {
  const [crawler, relative4] = getCrawler(globInput, options);
  return crawler ? formatPaths(await crawler.withPromise(), relative4) : [];
}
...
function trough() {
  ...
  function run2(...values) {
    ...
    const callback2 = values.pop();
    if (typeof callback2 !== "function") {
      throw new TypeError("Expected function as last argument, not " + callback2);
    }
    next(null, ...values);
...
function wrap(middleware, callback2) {
  let called;
  return wrapped;
...
function markdownTable(table, options) {
  ...
  while (++rowIndex < table.length) {
    ...
    while (++columnIndex2 < table[rowIndex].length) {
      const cell2 = serialize(table[rowIndex][columnIndex2]);
...
function parseMarkdown(source) {
  ...
  visit(tree, (node2) => {
    ...
    const column = node2.position?.start.column ?? 0;
    switch (node2.type) {
      case "link":
      case "image": {
        links.push({
          url: node2.url,
          kind: node2.type === "image" ? "image" : "link",
          line,
          column
        });
        break;
      ...
      case "definition": {
        links.push({ url: node2.url, kind: "definition", line, column });
        break;
      }
      case "heading": {
        anchors.add(slugger.slug(toString(node2)));
        break;
      }
      case "html": {
        extractFromHtml(node2.value, line, links, anchors);
        break;
      }
    }
  });
...
function extractFromHtml(html2, line, links, anchors) {
  HTML_TAG_RE.lastIndex = 0;
  let tagMatch;
  while ((tagMatch = HTML_TAG_RE.exec(html2)) !== null) {
...
function splitLocalTarget(rawUrl) {
  const hashIndex = rawUrl.indexOf("#");
  const beforeHash = hashIndex === -1 ? rawUrl : rawUrl.slice(0, hashIndex);
  const anchor = hashIndex === -1 ? void 0 : rawUrl.slice(hashIndex + 1);
  const queryIndex = beforeHash.indexOf("?");
  const path2 = queryIndex === -1 ? beforeHash : beforeHash.slice(0, queryIndex);
  return { path: path2, anchor: anchor === "" ? void 0 : anchor };
}

// src/resolve.ts
import { stat as stat2 } from "fs/promises";
import { dirname as dirname2, extname, join, relative as relative2, resolve as resolve3 } from "path";
...
function isMarkdownPath(path2) {
  return MARKDOWN_EXTENSIONS.has(extname(path2).toLowerCase());
}
...
async function checkLocalLink(rawUrl, ctx) {
  const { path: rawPath, anchor: rawAnchor } = splitLocalTarget(rawUrl);
  const path2 = safeDecode(rawPath);
  const anchor = rawAnchor === void 0 ? void 0 : safeDecode(rawAnchor);
  if (path2 === "") {
    if (anchor === void 0) return { status: "ok" };
  ...
  const targetAbs = path2.startsWith("/") ? join(ctx.base, path2) : resolve3(dirname2(ctx.fromFile), path2);
  let stats;
  try {
    stats = await stat2(targetAbs);
  } catch {
    return {
      status: "broken",
  ...
  if (anchor === void 0 || !ctx.checkAnchors) {
    return { status: "ok" };
  }
...
var HEAD_FALLBACK_STATUSES = /* @__PURE__ */ new Set([403, 405, 406, 501, 999]);
var STATUS_TEXT = {
  400: "Bad Request",
  401: "Unauthorized",
  403: "Forbidden",
...
function statusReason(status) {
  const text5 = STATUS_TEXT[status];
  return text5 ? `HTTP ${status} ${text5}` : `HTTP ${status}`;
}
var defaultSleep = (ms) => new Promise((res) => setTimeout(res, ms));
async function probe(url, method, options) {
...
async function checkOne(url, options) {
  ...
  for (let attempt = 0; attempt <= options.retries; attempt++) {
    ...
    if ("error" in result) {
      ...
      result = getResult;
    } else if (!isOk(result.status, options.okStatuses) && shouldFallbackToGet(result.status)) {
      const getResult = await probe(url, "GET", options);
...
async function mapPool(items, limit, fn) {
  ...
  async function worker() {
    while (cursor < items.length) {
      const index2 = cursor++;
      results[index2] = await fn(items[index2], index2);
    }
  }
  await Promise.all(Array.from({ length: workers }, () => worker()));
  return results;
}
async function checkExternalLinks(urls, options) {
  const unique = [...new Set(urls)];
  const results = await mapPool(
    unique,
    options.concurrency,
    (url) => checkOne(url, options)
  );
  const map3 = /* @__PURE__ */ new Map();
  unique.forEach((url, i) => map3.set(url, results[i]));
  return map3;
}

// src/checker.ts
var LOCAL_CHECK_CONCURRENCY = 32;
async function check(patterns, config, options = {}) {
  ...
  const parsed = await Promise.all(
    files.map(async (relPath) => {
      const absPath = resolve4(cwd, relPath);
  ...
  const getTargetAnchors = (absPath) => {
    const cached = anchorCache.get(absPath);
    if (cached) return cached;
    const entry = readFile(absPath, "utf8").then((src) => parseMarkdown(src).anchors).catch(() => void 0);
    anchorCache.set(absPath, entry);
    return entry;
  };
  const isIgnored = config.ignore.length > 0 ? (0, import_picomatch2.default)(config.ignore, { dot: true }) : () => false;
  const results = [];
  const localTasks = [];
  const externalTasks = [];
  for (const file of parsed) {
    for (const link2 of file.links) {
      const cls = classifyLink(link2.url);
      const result = {
        file: file.relPath,
        url: link2.url,
        line: link2.line,
      ...
      switch (cls) {
        case "external": {
          if (config.offline) {
            ...
            externalTasks.push({ result, url: link2.url.trim() });
          }
          break;
        }
        case "local":
        case "anchor": {
          localTasks.push(async () => {
            const outcome = await checkLocalLink(link2.url, {
...
async function runLocalTasks(tasks) {
  let cursor = 0;
...
function buildReport(fileCount, results, durationMs) {
  const counts = {
    ok: 0,
    broken: 0,
    ignored: 0,
...
var CONFIG_FILENAME = ".linkrotrc.json";
var NUMBER_FIELDS = [
  "timeout",
  "concurrency",
  "retries",
...
var STRING_ARRAY_FIELDS = ["ignore", "exclude"];
function normalizeConfig(raw) {
  ...
  if (out.concurrency !== void 0 && out.concurrency < 1) {
  ...
  if (obj.okStatuses !== void 0) {
    if (!Array.isArray(obj.okStatuses) || !obj.okStatuses.every((n) => typeof n === "number" && Number.isInteger(n))) {
      throw new Error(
        'Config field "okStatuses" must be an array of integers.'
      );
    }
    out.okStatuses = obj.okStatuses;
  }
  if (obj.userAgent !== void 0) {
  ...
  for (const field of ["checkAnchors", "offline"]) {
    const value = obj[field];
    if (value === void 0) continue;
...
function resolveConfig(fileConfig, overrides, cwd) {
  const merged = { ...fileConfig, ...stripUndefined(overrides) };
  const base = merged.base ?? ".";
  return {
    ...DEFAULT_CONFIG,
    ...stripUndefined(merged),
    base: isAbsolute2(base) ? base : resolve5(cwd, base)
  };
}
...
async function loadConfigFile(cwd, configPath) {
  const explicit = configPath !== void 0;
  const path2 = configPath ? isAbsolute2(configPath) ? configPath : resolve5(cwd, configPath) : resolve5(cwd, CONFIG_FILENAME);
  let contents;
  try {
    contents = await readFile2(path2, "utf8");
  } catch (err) {
    if (!explicit && err.code === "ENOENT") {
      return { config: {}, path: void 0 };
    }
    throw new Error(`Could not read config file "${path2}": ${describe(err)}`);
  }
  let parsed;
  try {
    parsed = JSON.parse(contents);
  } catch (err) {
    throw new Error(`Invalid JSON in config file "${path2}": ${describe(err)}`);
  }
  return { config: normalizeConfig(parsed), path: path2 };
}
...
function createColorizer(enabled) {
  const fn = ((style, text5) => enabled ? `${CODES[style]}${text5}${CODES.reset}` : String(text5));
  fn.enabled = enabled;
  return fn;
}
function shouldUseColor(override, env, isTTY) {
  if (override !== void 0) return override;
  if (env.NO_COLOR !== void 0 && env.NO_COLOR !== "") return false;
  if (env.FORCE_COLOR !== void 0 && env.FORCE_COLOR !== "") return true;
  return isTTY;
}
...
function formatText(report2, options = {}) {
  const c = createColorizer(options.color ?? false);
  const broken = report2.results.filter((r) => r.status === "broken");
  const lines = [];
  if (broken.length > 0) {
  ...
  lines.push(summaryLine(report2, c));
  return lines.join("\n");
}
function summaryLine(report2, c) {
  const fileWord = report2.files === 1 ? "file" : "files";
  const checked = `checked ${report2.totalLinks} links across ${report2.files} ${fileWord}`;
  const extras = [];
  if (report2.ignored > 0) extras.push(`${report2.ignored} ignored`);
  if (report2.skipped > 0) extras.push(`${report2.skipped} skipped`);
  const extra = extras.length > 0 ? `, ${extras.join(", ")}` : "";
  const duration = ` in ${formatDuration(report2.durationMs)}`;
  if (report2.broken === 0) {
    return `${c("green", "\u2713")} No broken links \u2014 ${checked}${extra}${duration}`;
  }
  const brokenWord = report2.broken === 1 ? "broken link" : "broken links";
  return `${c("red", "\u2716")} ${report2.broken} ${brokenWord} \u2014 ${checked}${extra}${duration}`;
}

// src/reporters/json.ts
function formatJson(report2) {
  return JSON.stringify(
    {
      ok: report2.broken === 0,
      summary: {
        files: report2.files,
        totalLinks: report2.totalLinks,
        ok: report2.ok,
        broken: report2.broken,
        ignored: report2.ignored,
        skipped: report2.skipped,
        durationMs: report2.durationMs
      },
      results: report2.results.map((result) => ({
        file: result.file,
        url: result.url,
        line: result.line,
...
function code3(value) {
  const safe = value.replace(/`/g, "\u200B`\u200B");
  return `\`${cell(safe)}\``;
}
function groupByFile2(results) {
  const groups = /* @__PURE__ */ new Map();
  for (const result of results) {
    const existing = groups.get(result.file);
    if (existing) existing.push(result);
...
function formatMarkdown(report2) {
  const lines = ["## \u{1F517} link-rot", ""];
  const broken = report2.results.filter((r) => r.status === "broken");
  const fileWord = report2.files === 1 ? "file" : "files";
  const checked = `checked **${report2.totalLinks}** links across **${report2.files}** ${fileWord}`;
  if (broken.length === 0) {
    ...
    const groups = groupByFile2(broken);
    ...
    for (const [file, fileResults] of groups) {
      ...
      for (const result of fileResults) {
        ...
        lines.push(`| ${result.line} | ${code3(result.url)} | ${problem} |`);
      }
      lines.push("");
    }
  }
  const extras = [
    `${report2.files} ${fileWord}`,
    `${report2.totalLinks} links`
  ];
  if (report2.ignored > 0) extras.push(`${report2.ignored} ignored`);
  if (report2.skipped > 0) extras.push(`${report2.skipped} skipped`);
...
function formatReport(report2, format, options = {}) {
  switch (format) {
    case "json":
      return formatJson(report2);
    case "markdown":
      return formatMarkdown(report2);
    case "text":
    default:
      return formatText(report2, { color: options.color });
  }
}
...
var EXIT_BROKEN = 1;
var EXIT_USAGE = 2;
var HELP = `link-rot v${VERSION} \u2014 find dead links in Markdown and docs
...
`;
function parseInteger(name, value, errors) {
...
async function run(argv, io) {
  ...
  try {
    ...
    io.writeErr(`link-rot: ${describe2(err)}
`);
...
`);
  ...
  const format = values.format === void 0 ? "text" : values.format;
...
`
  ...
  const errors = [];
  const timeout = parseInteger("timeout", values.timeout, errors);
  ...
  for (const raw of values["ok-status"] ?? []) {
    ...
    if (code4 !== void 0) cliOkStatuses.push(code4);
  }
  if (concurrency === 0) {
...
`);
  ...
  let fileConfig;
  try {
    const loaded = await loadConfigFile(io.cwd, values.config);
...
`);
  ...
  const overrides = {
    offline: values.offline ? true : void 0,
    timeout,
    concurrency,
    retries,
    base: values.base,
  ...
  let report2;
  try {
    report2 = await check(globs, config, { cwd: io.cwd });
  } catch (err) {
    io.writeErr(`link-rot: ${describe2(err)}
`);
    return EXIT_USAGE;
...
`
  ...
  const colorOverride = values.color ? true : values["no-color"] ? false : void 0;
  const color2 = format === "text" && shouldUseColor(colorOverride, io.env, io.isTTY);
  io.write(formatReport(report2, format, { color: color2 }) + "\n");
  if (report2.broken > 0 && !values["exit-zero"]) return EXIT_BROKEN;
  return EXIT_OK;
}
function renderCommand(positionals, values, io) {
  if (typeof values.format === "string" && !isReportFormat(values.format)) {
    io.writeErr(
      `link-rot: invalid --format "${values.format}" (expected ${REPORT_FORMATS.join(", ")}).
`
...
`);
  ...
  let report2;
  try {
    report2 = reportFromJson(JSON.parse(contents));
  } catch (err) {
    io.writeErr(`link-rot: could not parse JSON report: ${describe2(err)}
`);
    return EXIT_USAGE;
  }
  const colorOverride = values.color ? true : values["no-color"] ? false : void 0;
...
// src/cli.ts
run(process2.argv.slice(2), {
  cwd: process2.cwd(),
  env: process2.env,
  isTTY: Boolean(process2.stdout.isTTY),
  write: (text5) => process2.stdout.write(text5),
  writeErr: (text5) => process2.stderr.write(text5),
  readStdin: () => readFileSync2(0, "utf8")
}).then((code4) => {
  process2.exitCode = code4;
}).catch((err) => {
  const message = err instanceof Error ? err.stack ?? err.message : err;
  process2.stderr.write(`link-rot: unexpected error
${message}
`);
  process2.exitCode = EXIT_USAGE;
});
